...
/**
 * Merge model static ACLs
 *
 * So that a child class can override ALLOW/DENY
 *
 * @param {Object} target The target acls
 * @param {Object} src The source acls
 * @returns {Object} The merged acl
 */
function mergeACLs(baseacls, subacls) {
    var merged = [];
    baseacls = baseacls || [];
    subacls = subacls || [];
    merged = merged.concat(baseacls);
    subacls.forEach(function (sub) {
        var matchIndex = -1;
        var matches = merged.some(function (m, index, ar) {
            if (m.property === sub.property &&
                m.principalType === sub.principalType &&
                m.principalId === sub.principalId &&
                m.accessType === sub.accessType) {
                matchIndex = index;
                return true;
            }
        });
