...
function isInstanceQuery(model, ctx, filter) {
    ...
    if (pkValue !== undefined && !Array.isArray(pkValue)) {
...
function removeById(modelName, id, scope) {
    return null;
    assert(id);
    var cache = instanceCache;
    if (!cache[modelName]) {
...
CacheMixin.evictCache = function(evictInstanceCache, evictCtx) {
    if (!evictInstanceCache && evictCtx) {
        removeById(this.modelName, evictCtx.id, evictCtx.scope);
    }
...
CacheMixin.clearCacheOnDelete = function(ctx, cb) {
    ...
    if (!ctx.id) {
        ...
        removeById(model.modelName, ctx.id, ctx.instance._scope);
        evictInstanceCache = false;
    }
