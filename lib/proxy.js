...
ProxyMixin.invokeProxyIfRemote = function invokeProxyIfRemote(methodName) {
    ...
    var model = typeof(this) === 'function' ? this : this.__proto__.constructor;
    ...
    } else if (this.settings.proxyMethods) {
        if (this.settings.proxyMethods.some(m => m.name === restMethod.name)) {
            if (restMethod.isStatic === false) {
                var data = args[0];
                id = getIdValue(model, data);
                ctorArgs = [id];
            } else {
                var proxyIdSource = model.settings.proxyMethods.find(m => {
                    return m.name === restMethod.name;
                }).idSource || 0;
