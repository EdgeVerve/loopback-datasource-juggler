...
ProxyMixin.invokeProxyIfRemote = function invokeProxyIfRemote(methodName) {
    ...
    req.headers['x-evproxy-model-plural'] = model.clientPlural.toLowerCase();
    req.headers['x-evproxy-model-id'] = id;
    var accessObj = {};
    accessObj.accessTokenData = options.accessTokenData;
   // var callContextHeader = Buffer.from(JSON.stringify(options), 'utf8').toString('base64');
   var callContextHeader = Buffer.from(JSON.stringify(accessObj), 'utf8').toString('base64');
    req.headers['x-evproxy-context'] = callContextHeader;
    var self = this;

    invocation.invoke(function(err) {
      // console.log("invoke time " ,new Date() - bef);
...
ProxyMixin.checkAccess = function(token, modelId, sharedMethod, ctx, callback) {
    ...
    // Do not check ACL if Model definition settings has a property "bypassAthorization" set to true.
    if (this.definition && this.definition.settings && this.definition.settings.bypassAthorization 
        && (this.definition.settings.bypassAthorization.toString() === 'true')) {
