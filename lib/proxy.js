...
var socket = model.app.get('oe-tx-router');
 ...
 var ctorArgs;
    var restMethod;
        ...
        if (restMethod) {
            ;//console.log('restMethod ', model.modelName, methodName, restMethod.name, restMethod.isStatic, restMethod.ctor)
        } else {
            ;//console.log('no rest mdethod ', model.modelName, methodName);
        }
 		if (!restMethod) {
    ...
    if (model.clientPlural.toLowerCase() === options.evproxyModelPlural &&
        id == options.evproxyModelId) {
            console.log('Processing myself ----->', model.modelName + '/' + id)
        return false;
    }
    ...
    restMethod.accepts.forEach(function(item, i){
     ...
     var data = {
        ...
        ctorArgs : ctorArgs,
        args : namedArgs,
        callContextHeader : Buffer.from(JSON.stringify(options), 'utf8').toString('base64')
    };
    ...
    console.log('Forwarding request to ----->', model.modelName + '/' + id)

    c.transieve(data, function(stateResponse) {
        console.log('Received Response from Remote ->', model.modelName + '/' + id)
        var d = new Date();
        var t2 = d.getTime();
        timer_proxyCall.push(t2-t1);
        printAvg(timer_proxyCall, 'timer_proxyCall');
    ...
    // socket.emit("process", data, function(stateResponse) {
    //     var d = new Date();
    //     var t2 = d.getTime();
    //     timer_proxyCall.push(t2-t1);
    //     printAvg(timer_proxyCall, 'timer_proxyCall');
