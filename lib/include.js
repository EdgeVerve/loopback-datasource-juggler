...
Inclusion.include = function (objects, include, options, cb) {
  ...
  function processIncludeItem(objs, include, options, cb) {
    ...
    function includeHasManyThrough(callback) {
       ...
       */
      if (relation.modelThrough.getOverridenModel) {
        var modelThrough2 = relation.modelThrough.getOverridenModel(options);
        modelThrough2.find(throughFilter, options, throughFetchHandler);
      }
      else {
      ...
      function throughFetchHandler(err, throughObjs) {
         ...
         */
        if (relation.modelTo.getOverridenModel) {
          var model2 = relation.modelTo.getOverridenModel(options);
          model2.find(filter, options, targetsFetchHandler);
        }
        else {
    ...
    function includeReferencesMany(callback) {
      ...
      if (relation.modelTo.getOverridenModel) {
        var model2 = relation.modelTo.getOverridenModel(options);
        model2.find(filter, options, targetFetchHandler);
      }
      else {
    ...
    function includePolymorphicBelongsTo(callback) {
      ...
      function processPolymorphicType(modelType, callback) {
        ...
        if (Model.getOverridenModel) {
          var model2 = Model.getOverridenModel(options);
          model2.find(typeFilter, options, targetFetchHandler);
        }
        else {
