...
var mergeACLs = require('./utils').mergeACLs;
...
ModelBuilder.prototype.registerCustomType = function(customType, type, options) {
	var cfg = {type:type};
	var obj = mergeSettings(options, cfg);
...
ModelBuilder.prototype.define = function defineClass(className, properties, settings, parent) {
  ...
  ModelClass.extend = function (className, subclassProperties, subclassSettings) {
    ...
    subclassSettings = subclassSettings || {};
		...
		var ownDefinition = _.cloneDeep(subclassSettings);
		ownDefinition.name = className;
		ownDefinition.properties = _.cloneDeep(subclassProperties);
    ...
    // subclass should be able to override baseclass acls
    // mergeSettings is a generic method, which simply concatenates
    // arrays, so using a special method for acls
    var acls = mergeACLs(settings.acls, subclassSettings.acls);
    subclassSettings.acls = [];
    // Merge the settings
    var originalSubclassSettings = subclassSettings;
    subclassSettings = mergeSettings(settings, subclassSettings);
    subclassSettings.acls = acls;
...
ModelBuilder.prototype.defineProperty = function (model, propertyName, propertyDefinition) {
  if (propertyDefinition.type && 
	  ...
	  this.customTypes[propertyDefinition.type.toLowerCase()]) {
