...
function validateCustom(attr, conf, err, done) {
  conf.customValidator.call(this, err, done, attr, conf);
}
...
function validateUniqueness(attr, conf, err, done) {
  ...
  this.constructor.find(cond, conf.options.callContext, function (error, found) {
    if (error) {
      err(error);
 ...
 */
Validatable.prototype.isValid = function (callback, context, data) {
  if (data === undefined) {
    data = context;
    context = {options:{}};
  }
