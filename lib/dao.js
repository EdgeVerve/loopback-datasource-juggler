...
DataAccessObject.find = function find(query, options, cb) {
    ...
    var allCb = function (err, data, next) {
        var nextCb = cb;
    ...
    if (options.notify === false) {
        ...
        this.notifyObserversOf('access', context, function (err, ctx) {
            ...
            if (cache.getFromCache(self, originalFilter)) {
                    ...
                    connector.all.length === 4 ?
                        connector.all(self.modelName, ctx.query, options,
                            function (err, objs) {
                                ctx.accdata = objs;
                                self.notifyObserversOf('after accesss', ctx, function(err, ctx) {
                                    allCb(err, ctx.accdata, function(err, objs) {
                                        async.each(objs, function (instance, callback) {
                                            if( typeof instance.init === 'function') {
                                                instance.init(ctx, callback);
                                            } else {
                                                callback();
                                            }
                                        }, function(err) {
                ...
                .catch(function(err){
                    ...
                    cache.remove(self.modelName, originalFilter);
                    cb(err);
                });
