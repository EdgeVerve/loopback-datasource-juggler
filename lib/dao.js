...
DataAccessObject.instanceLocker = function() {
    if (!this._instanceLocker) {
            console.log('create a _instanceLocker for ' , this.modelName);
            this._instanceLocker = new AsyncLock();
    }
...
DataAccessObject.create = function (data, options, cb) {
    ...
    Model.checkIdempotency(context, function (err, rinst) {
...
DataAccessObject.updateOrCreate = DataAccessObject.upsert = function upsert(data, options, cb) {
    ...
    var find2cb = function (err, inst) {
        ...
        if (mixins && mixins.DataPersonalizationMixin) {
            ...
            if (!ignoreAutoScope && inst._autoScope.tenantId) {
...
DataAccessObject.find = function find(query, options, cb) {
    ...
    if (options.notify === false) {
        ...
        this.notifyObserversOf('access', context, function (err, ctx) {
            ...
            var promise = cache.getFromCache(self, ctx, originalFilter);
            if (promise) {
                debug('EV_CACHE', 'going to cache for ', self.modelName, 'with query', originalFilter);
                promise.then(function resolveCachePromise(objs) {
                    beforeAllCb(null, objs);
                }).catch(function (err) {
                    beforeAllCb(err);
                });
            } else {
                ...
                cache.cache(self, ctx, originalFilter, promise);
...
DataAccessObject.remove = DataAccessObject.deleteAll = DataAccessObject.destroyAll = function destroyAll(where, options, cb) {
    ...
    if (options.notify === false) {
        ...
        Model.checkIdempotencyForDelete(context, function (err, rinst) {
            if (err) {
                return cb(err);
            }
            if (rinst) {
...
DataAccessObject.removeById = DataAccessObject.destroyById = DataAccessObject.deleteById = DataAccessObject.deleteWithVersion = function deleteById(id, version, options, cb) {
    ...
    if (version) {
        ...
        var where = {};
        where[idName] = id;
        where._version = version;
        // call destroyAll to delete the record.
        Model.destroyAll(where, options, function deleteWithVersionCB(err, info) {
            if (err) {
                return cb(err);
            } else if (info.count === 0) {
                var err1 = new Error('No record found');
    ...
    Model.checkIdempotencyForDelete(context, function (err, rinst) {
        ...
        Model.remove(byIdQuery(Model, id).where, options, function (err, info) {
            if (err) return cb(err);
...
DataAccessObject.prototype.updateAttributes = function updateAttributes(data, options, cb) {
    ...
    Model.checkIdempotency(context, function (err, rinst) {
        ...
        if (err) {
            console.log('checkIdempotency error ', err);
            return cb(err);
        }
        if (rinst) {
            return cb(err, rinst);
        ...
        if (lock.isBusy(key)) {
                                console.log('Yes it is busy');
                                var error = new Error('Lock is already caught');
                                //err.retriable = false;
                                return cb(error);
        } else {
                console.log('No it is not busy');
                lock.acquire(key, function(cb) {
                    console.log('after lock acquire');
         Model.notifyObserversOf('before save', context, function (err, ctx) {
            if (err) return cb(err);
                ...
                inst.trigger('save', function (saveDone) {
                    inst.trigger('update', function (done) {
                        ...
                        Model.notifyObserversOf('persist', ctx, function (err) {
                            ...
                            } else if (Model.settings._versioning) {
            ...
            console.log('after lock is released');
            cb(err, inst)
        });
        };
    });
...
DataAccessObject.prototype.lock = function lock(modelName, newTransactionId, cb) {
    var transactionId = cache.getLock(modelName, this.id);
    if (newTransactionId === null || newTransactionId === undefined) {
        return cb(new Error('transaction id not provided to lock'));
    }
    if (transactionId === undefined || transactionId === null) {
        cache.setLock(modelName, this.id, newTransactionId);
        return cb();
    }
    if (transactionId === newTransactionId) {
        return cb();
    } else {
        return cb(new Error(modelName + ' instance is already locked'));
    }
};

DataAccessObject.prototype.unlock = function unlock(modelName, newTransactionId, cb) {
    if (newTransactionId === null || newTransactionId === undefined) {
        return cb(new Error('transaction id not provided to unlock'));
    }
    var transactionId = cache.getLock(modelName, this.id);
    if (transactionId === newTransactionId) {
        cache.setLock(modelName, this.id, null);
    }
    return cb();
}
...
//DataAccessObject.observe('after delete', function clearCache(ctx, next) {
//    var evictInstanceCache;
//    //TODO: check ctx.where for id and evict instance cache accordingly
//    if (ctx.data) {
