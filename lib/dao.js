                    ...
                    if (rowStatus == 'modified') {
                        ...
                        if (w.length == 1) {
                            ...
                            model.deleteById(whereClause, options, function (err, instance) {
                                cb(err/*, instance*/); //do not return instance for delete operation.
                            });
                        } else {
                            var whereClause = {
                                and: w
...
function implicitComposite(self, data, childData, relations, options, cb) {
    ...
    function createModelChildData(self, data, childData, relations, options, deleteRelations, cb) {
        ...
        function createHasOneChildModelRecord(childModel2, cr, options, done4, relationName) {
            try {
                childModel2.create(cr, options, function (err3, createdRecord2) {
                    ...
                    responseData.__data[relationName] = createdRecord2;
                    done4(err3);
                });
            } catch (err) {
                done4(err);
            ...
            self.create(data, options, function (err, createdRecord) {
                ...
                async.forEachOfSeries(childData, function (childRows, relationName, done3) {
                    ...
                    if(self.relations[relationName].type === 'hasOne' && !Array.isArray(childRows)){
                        childRows[keyTo] = createdRecord[keyFrom];
                        createHasOneChildModelRecord(childModel2, childRows, options, done3, relationName);
                    } else {
                        async.eachSeries(childRows, function (cr, done4) {
                            cr[keyTo] = createdRecord[keyFrom];
        ...
        try {
            self.updateOrCreate(data, options, function (err, createdRecord) {
                ...
                async.forEachOfSeries(childData, function (childRows, relationName, done3) {
                    ...
                    if(self.relations[relationName].type === 'hasOne' && !Array.isArray(childRows)){
                        childRows[keyTo] = createdRecord[keyFrom];
                        childRows.__row_status = childRows.__row_status || 'modified';
                        var rowStatus = childRows.__row_status;
                        applyRecord(childModel, pk2, options, childRows, function (err3, createdRecord2) {
                            if (err3)
                                return done3(err3);
                            if (!createdRecord2)
                                return done3();
    ...
    function createOrUpdateModelChildData2(self, data, childData, relations, options, deleteRelations, cb) {
        ...
        self.updateAttributes(data, options, function (err, createdRecord) {
            ...
            async.forEachOfSeries(childData, function (childRows, relationName, done3) {
                ...
                if(self.constructor.relations[relationName].type === 'hasOne' && !Array.isArray(childRows)){
                    childRows[keyTo] = createdRecord[keyFrom];
                    childRows.__row_status = childRows.__row_status || 'modified';
                    var rowStatus = childRows.__row_status;
                    applyRecord(childModel, pk2, options, childRows, function (err3, createdRecord2) {
                ...
                inst.isValid(function (valid) {
                    ...
                    inst.trigger('save', function (saveDone) {
                        inst.trigger('update', function (done) {
                            ...
                            var checkOnError = function(err, context) {
                                return Model.checkIdempotencyAfter(err, context, function (error, obj) {
                                    if (error) {
                                        return cb(error);
                                    }
                            ...
                            function updateAttributesCallback(err, info) {
                                if (Model.settings._versioning &&
                                    ...
                                    err.retriable = false;
                                    err.status = 422;
                                    return checkOnError(err, context);
                                }
                                if (err) {
                                    return cb(err);
                                }
