...
DataAccessObject.updateOrCreate = DataAccessObject.upsert = function upsert(data, options, cb) {
    ...
    var proxyctx = { methodName: 'upsert', id: data.id };
    if (this.isRemoteInstance(proxyctx, options)) {
        var args = [data, options];
        return this.invokeProxy(proxyctx, args, cb);
    }
    ...
    if (mixins && mixins.DataPersonalizationMixin) {
        ...
        Model.findOne(idQuery, options, function (err, origdata) {
            if (origdata) {
                ...
                } else if (mixins && mixins.IdempotentMixin) {
                    Model.notifyObserversOf('access', context, doUpdateOrCreate);
                } else if (data.scope && !(_.isEqual(dataScope, resultScope))) {
                ...
                function done(err, data, info) {
                    ...
                    Model.notifyObserversOf('loaded', context, function (err) {
                        ...
                        if (err) {
                            ...
                            cache.update(Model, ctx.where, data);
                            Model.notifyObserversOf('after save', context, function (err) {
                                cb(err, obj);
                                if (!err) {
                                    Model.emit('changed', inst);
...
DataAccessObject.findById = function findById(id, filter, options, cb) {
    ...
    var proxyctx = { methodName: 'findById', id: id }
    if (this.isRemoteInstance(proxyctx, options)) {
        var args = [id, filter, options];
        return this.invokeProxy(proxyctx, args, cb);
    }
...
DataAccessObject.find = function find(query, options, cb) {
    ...
    if (options.notify === false) {
        ...
        this.notifyObserversOf('access', context, function (err, ctx) {
            ...
            var promise = cache.getFromCache(self, originalFilter);
            if (promise) {
...
DataAccessObject.remove = DataAccessObject.deleteAll = DataAccessObject.destroyAll = function destroyAll(where, options, cb) {
    ...
    function doDelete(where) {
        ...
        function done(err, info) {
            ...
            cache.remove(Model, { where: context.where });
...
DataAccessObject.prototype.updateAttributes = function updateAttributes(data, options, cb) {
    ...
    var proxyctx = { methodName: 'updateAttributes', id: firstId }
    if (Model.isRemoteInstance(proxyctx, options)) {
        var args = [data, options];
        return Model.invokeProxy(proxyctx, args, cb);
    }
...
DataAccessObject.prototype.lock = function lock(newTransactionId, cb) {
    var transactionId = cache.getLock(this.name, this.id);
    if (newTransactionId === null || newTransactionId === undefined) {
        return cb(new Error('transaction id not provided to lock'));
    }
    if (transactionId === undefined || transactionId === null) {
        cache.setLock(this.name, this.id, newTransactionId);
        return cb();
    }
    if (transactionId === newTransactionId) {
        return cb();
    } else {
        return cb(new Error(this.name +' instance is already locked'));
    }
};

DataAccessObject.prototype.unlock = function unlock(newTransactionId, cb) {
    if (newTransactionId === null || newTransactionId === undefined) {
        return cb(new Error('transaction id not provided to unlock'));
    }
    var transactionId = cache.getLock(this.name, this.id);
    if (transactionId === newTransactionId) {
        cache.setLock(this.name, this.id, null);
    }
    return cb();
}
